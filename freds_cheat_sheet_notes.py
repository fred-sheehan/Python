# lots of useful snippets for various things. Shamelessly grabbed from
# the following URL and reformatted to better fit a code editor. It will
# look better if you have syntax highlighting, but has been well indented
# in case you don't
...
# 10. Reading and Writing Binary Files
        # To read from and write to a file in binary mode
...
# 8. Using Sessions with Requests
        # To use a session object for making multiple requests
        # to the same host, which can improve performance:
import requests
...
# 10. Streaming Large Responses
        # To stream a large response to process it in chunks,
...
# 5. Popping an Element from a List
        # To remove and return an element at a given index
...
# 8. List Comprehension
        # To create a new list by applying an expression
        # to each element of an existing one:
...
# 8. Dictionary Comprehension
        # To conjure a new dictionary through an incantation
        # over an iterable:
squares = {x: x**2 for x in range(5)}


# 9. Merging Dictionaries
        # To merge two or more dictionaries, forming a new
        # alliance of their entries:
alchemists = {'Paracelsus': 'Mercury'}
philosophers = {'Plato': 'Aether'}
merged = {**alchemists, **philosophers}

# 10. Getting a Value with Default
        # To retrieve a value safely, providing a default for absent
        # keys: Returns 'Unknown' if 'Neon' is not found
element = elements.get('Neon', 'Unknown')
...
# 1. Navigating File Paths
        # To craft and dissect paths, ensuring compatibility across
        # realms (operating systems):
import os
...
# 7. Changing the Current Working Directory
        # To shift your presence to another directory within
        # the filesystem:
import os
...
# 8. Path Existence and Type
        # To discern the existence of paths and their nature —
        # be they file or directory:
import os
...
# 9. Working with Temporary Files
        # To summon temporary files and directories,
...
# 10. Getting System Information
        # To unveil information about the host system, its name,
        # and the enchantments it supports:
import os
import platform
...
# 2. Creating a Cursor
        # To create a database cursor, enabling the traversal and
...
# 5. Asynchronous Iteration
        # To traverse through asynchronously, allowing time
        # for other functions in between:
...
# 6. Using Asynchronous Context Managers
        # To ensure resources are managed within the bounds of
...
# 10. Event Loops
        # To directly engage with the asynchronous loop,
        # customizing the flow of execution:
...
# 5. Filtering Rows
        # To sift through the DataFrame, selecting rows
...
# 7. Grouping and Aggregating Data
        # To gather your data into groups and extract new data
        # through aggregation:
...
# 10. Pivoting and Reshaping Data
        # To transmute the shape of your DataFrame, revealing hidden
        # patterns and structures with a pivot operation:
...
result = np.dot(a.reshape(1, 3), b.reshape(3, 1))
...
# 9. Boolean Indexing
        # To filter the elements of an array through the sieve
        # of conditionals:
filtered = a[a > 2]
...
# 2. Adding Titles and Labels
        # To create names for axes and title your plot to
        # give better context:
plt.plot(x, y)
...
# 9. Customizing Ticks
        # To create your own marks upon the axes, defining
        # the scale of your values:
plt.plot(x, y)
...
# 2. Splitting Data into Training and Test Sets
        # To divide your data, dedicating portions to
...
# 7. Feature Scaling
        # To create the appropriate scales of your features,
...
# 8. Parameter Tuning with Grid Search
        # To refine your model’s parameters, seeking the
        # optimal combination:
...
# 9. Pipeline Creation
        # To streamline your data processing and modeling steps,
...
# 2. Creating Specific Date and Time
        # To conjure a moment from the past or future,
        # crafting it with precision:
...
# 5. Working with Time Deltas
        # To traverse the distances between moments, leaping
...
# 10. Working with Unix Timestamps
        # To converse with the ancient epochs, translating
        # their count from the dawn of Unix:
...
# 3. List Comprehensions with Multiple Iterables
        # To merge and transform elements from multiple source
        #  in a single dance:
pairs = [(x, y) for x in [1, 2, 3] for y in [3, 1, 4] if x != y]
print(pairs)  # Pairs of non-equal elements

# 4. Using Lambda Functions
        # To summon anonymous functions, ephemeral and concise,
        # for a single act of magic:
square = lambda x: x**2
print(square(5))
...
# 6. List Comprehensions for Flattening Lists
        # To flatten a nested list, spreading its elements
...
# 9. Class Methods and Static Methods
        # To bind actions to the class itself or liberate them
        # from the instance, serving broader purposes:
class Enchanter:
    @staticmethod
    def enchant(item):
...
# 10. Properties and Setters
        # To elegantly manage access to an entity’s attributes,
        # guiding their use and protection:
class Elementalist:
...
# 9. Using Directives
        # To dynamically include or skip fields in your queries
        # based on conditions:
query = gql('''
...
# 10. Batching Requests
        # To combine multiple operations into a single request,
...
all_matches = re.findall(r"t\w+", text)
print(all_matches)
...
escaped = re.search(r"\bfor\b", text)
...
# 10. Lookahead and Lookbehind Assertions
        # To match a pattern based on what comes before or
        # after it without including it in the result:
lookahead = re.search(r"\b\w+(?= string)", text)
lookbehind = re.search(r"(?<=Search )\w+", text)
if lookahead:
...
# 11. Flags to Modify Pattern Matching Behavior
        # To use flags like re.IGNORECASE to change how
...
# 13. Matching Across Multiple Lines
        # To match patterns over multiple lines using the
        # re.MULTILINE flag:
...
# 14. Lazy Quantifiers
        # To match as few characters as possible using
        # lazy quantifiers (*?, +?, ??):
...
# 5. String Methods — strip, rstrip, lstrip
        # To remove whitespace or specific characters from
...
# 11. String Methods — isdigit, isalpha, isalnum
        ...
        # alphabetic characters, or alphanumeric characters:
print("123".isdigit())
print("abc".isalpha())
...
# 2. Listing Installed Packages
        # To survey the compendium of libraries that reside
        # within your realm, noting their versions and lineage:
pip list

# 3. Upgrading a Package
        # To imbue an installed library with enhanced powers
        # and capabilities, elevating it to its latest form:
...
# 8. Installing Packages from a Requirements File
        # To conjure a symphony of libraries in unison, each
        # aligned with the notations in your tome of requirements:
...
# 10. subprocess - Subprocess Management
        # To spawn new processes and connect to their
...
# 14. argparse - Parser for Command-line Options,
...
# 19. collections - Container Data Types
        # To use specialized container data types (deque,
...
# 31. shutil - High-level File Operations
        # To perform high-level file operations,
        # like copying and archiving:
import shutil
...
# 36. ssl - TLS/SSL Wrapper for Socket Objects
        # To handle TLS/SSL encryption and peer authentication
        # for network sockets:
import ssl
...
# 39. email - Managing Email Messages
        # To manage email messages, including MIME and
        # other RFC 2822-based message documents:
...
# 44. secrets - Generate Secure Random Numbers for Managing Secrets
        # To generate secure random numbers for managing secrets,
        # such as tokens or passwords:
import secrets
...
# 48. tarfile - Read and Write Tar Archive Files
        # To work with tar archive files, allowing you to archive
